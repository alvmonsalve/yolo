...
# Contours for speed estimation
CONTOUR_FACTORS_TO_MILE = [0.00014954876635204505, 4.185501423070484e-06, 0.00014456399097356545, 3.966365222909725e-06, 0.00011616161616161616]
...
# PURPOSE: Identifying if the current box was present in the previous frames And Estimate the Instantaneous Speed
...
def boxInPreviousFrames(previous_frame_detections, current_box, current_detections):
	...
	# Keeping the vehicle ID
	current_detections[(centerX, centerY)] = previous_frame_detections[frame_num][coord] 	#Re-assign previous_frame_detections to current_detections
	
	#Calculate Instantaneous Speed here
	newspeed = calculate_instantaneous_speed(prevCenterX=coord[0], prevCenterY=coord[1], centerX=centerX, centerY=centerY)
	current_detections[(centerX, centerY)][1].append(newspeed)	# Add the new speed to the Ring Speed Buffer

	return True

# PURPOSE: Calculate the instantaneous Speed between consecutive frames
# PARAMETERS: Center of detected vehicle
# RETURN: Speed
def calculate_instantaneous_speed(prevCenterX, prevCenterY, centerX, centerY):
	speed = np.float(0) # initialisation

	# Check if both frames are in the same Contour
	for idx in range(len(CONTOURS)):
		if cv2.pointPolygonTest(CONTOURS[idx], (centerX, centerY), False) >= 0 and cv2.pointPolygonTest(CONTOURS[idx], (prevCenterX, prevCenterY), False) >= 0:
			dy_miles = (centerY-prevCenterY) * CONTOUR_FACTORS_TO_MILE[idx]
			speed = dy_miles * VIDEO_FPS * 3600
			break

	return speed


def count_vehicles_and_speed(idxs, boxes, classIDs, vehicle_count, previous_frame_detections, frame):
	current_detections = {}
	# ensure at least one detection exists
	if len(idxs) > 0:
		# loop over the indices we are keeping
		for i in idxs.flatten():
			...
			if (LABELS[classIDs[i]] in list_of_vehicles):
				...
				# Display the average speed at the centre of the box
				if np.average(ID[1]) > 0:
					cv2.putText(frame, "{:.2f}".format(np.average(ID[1])), (centerX, centerY+30),\
						cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0,0,255], 2)
...
previous_frame_detections = [{(0,0):[0, RingBuffer(capacity=RING_SPEED_SIZE, dtype=np.float)]} for i in range(FRAMES_BEFORE_CURRENT)]  # Initializing all trees
